...
ax1.set_title('Consumo, Setpoint, Temperaturas y Electroválvulas — 10 al 13 de Enero\n(SAC λ_T=25, Evaluación episodio 20)', fontsize=14, fontweight='bold')
...
for col, (label, color) in zones.items():
    ax3.plot(d['datetime'], d[col], color=color, linewidth=1.2, label=label, alpha=0.9)
...
for col, (label, color) in valves.items():
    ...
    pct = 100 * d[col].mean()
    ax4.text(d['datetime'].iloc[-1] + pd.Timedelta(hours=1), off + 0.35,
...
ax4.xaxis.set_major_formatter(mdates.DateFormatter('%d Ene %Hh'))
...
plt.tight_layout()
plt.savefig('/workspaces/sinergym/bomba_enero_10_13.png', dpi=150, bbox_inches='tight')
print("Gráfico guardado en /workspaces/sinergym/bomba_enero_10_13.png")

print(f"\n=== Estadísticas 10-13 Enero ===")
