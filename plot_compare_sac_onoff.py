...
for _, row in sac.iterrows():
    ...
    offsets.append(sum(months_days[i] for i in range(1, m)) + dd - 1)
sac['datetime'] = [base_date + pd.Timedelta(days=d, hours=int(sac.iloc[i]['hour'])) for i, d in enumerate(offsets)]
...
for dt_str in onoff['Date/Time']:
    ...
    if hour == 24:
        hour = 0
    try:
        dt_list.append(pd.Timestamp(2025, month, day, hour))
    except:
...
def plot_comparison(sac_slice, onoff_slice, title_suffix, date_fmt, fname):
    ...
    # --- Panel 1: Consumo bomba (SAC vs ON/OFF) ---
    ax1 = axes[0]
    ...
    # --- Panel 2: Setpoint bomba SAC ---
    ax2 = axes[1]
    ax2.step(sac_slice['datetime'], sac_slice['bomba'], where='post', color='#E91E63', linewidth=1.4, label='SAC setpoint')
    ...
    ax2.text(0.5, 0.92, f"SAC: {sac_slice['bomba'].min():.0f}-{sac_slice['bomba'].max():.0f}°C (media {sac_slice['bomba'].mean():.1f}°C)  |  ON/OFF: fijo por termostato",
    ...
    # --- Panel 3: Temperaturas Este y Oeste (SAC vs ON/OFF) + PMV band ---
    ax3 = axes[2]

    rh_avg_sac = sac_slice[['east_perimeter_air_humidity', 'west_perimeter_air_humidity']].mean(axis=1)
    T_lo_sac = T_from_pmv(-0.5, rh_avg_sac)
    T_hi_sac = T_from_pmv(0.5, rh_avg_sac)
    ax3.fill_between(sac_slice['datetime'], T_lo_sac, T_hi_sac, color='green', alpha=0.12,
    ...
    # --- Panel 4: PMV Este y Oeste (SAC vs ON/OFF) ---
    ax4 = axes[3]
    ...
    pct_sac_e = pct_comfort(sac_pmv_e.values)
    pct_sac_w = pct_comfort(sac_pmv_w.values)
    pct_onoff_e = pct_comfort(onoff_slice['pmv_east'].values)
    pct_onoff_w = pct_comfort(onoff_slice['pmv_west'].values)
    pct_text = (f"% en confort → Este: SAC {pct_sac_e:.0f}% / ON/OFF {pct_onoff_e:.0f}%  |  "
                f"Oeste: SAC {pct_sac_w:.0f}% / ON/OFF {pct_onoff_w:.0f}%")
    ...
    # --- Panel 5: Electroválvulas SAC ---
    ax5 = axes[4]
    valves = {
        'electrovalve_east':  ('Este', '#9C27B0'),
        'electrovalve_west':  ('Oeste', '#00BCD4'),
    }
    offsets_v = {'electrovalve_east': 1, 'electrovalve_west': 0}
    ...
    ax5.set_yticks([0.35, 1.35])
    ax5.set_yticklabels(['Oeste', 'Este'])
    ...
    for ax in axes:
        ax.set_xlim(sac_slice['datetime'].min(), sac_slice['datetime'].max())

    plt.tight_layout()
    plt.savefig(fname, dpi=150, bbox_inches='tight')
    print(f"Guardado: {fname}")
    print(f"  Consumo: SAC={kwh_sac:.0f} kWh vs ON/OFF={kwh_onoff:.0f} kWh")
    print(f"  Confort Este:  SAC={pct_sac_e:.0f}% vs ON/OFF={pct_onoff_e:.0f}%")
    print(f"  Confort Oeste: SAC={pct_sac_w:.0f}% vs ON/OFF={pct_onoff_w:.0f}%")
    plt.close()

# ===================== DÍAS MÁS CALUROSOS (7-9 Feb) =====================
...
print("=== 3 DÍAS MÁS CALUROSOS (7-9 Feb) ===")
plot_comparison(sac_hot, onoff_hot,
                '3 Días Más Calurosos: 7-9 Feb (T ext media 25.2°C)',
                '%d Feb %Hh',
                '/workspaces/sinergym/compare_hottest_3days.png')

# ===================== DÍAS MÁS FRÍOS (27-29 Jun) =====================
...
plot_comparison(sac_cold, onoff_cold,
                '3 Días Más Fríos: 27-29 Jun (T ext media 4.0°C)',
                '%d Jun %Hh',
                '/workspaces/sinergym/compare_coldest_3days.png')
