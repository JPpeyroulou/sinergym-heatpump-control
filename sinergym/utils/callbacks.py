...
class QValueCallback(BaseCallback):
    ...
    def _on_training_start(self) -> None:
        ...
        self.workspace_path = env.get_wrapper_attr('workspace_path')
        self.csv_path = os.path.join(self.workspace_path, 'qvalue_log.csv')

        if not os.path.exists(self.csv_path):
            with open(self.csv_path, mode='w', newline='') as file:
                writer = csv.writer(file)
                writer.writerow(['episode', 'q_mean', 'reward_sum'])

    def _on_rollout_end(self) -> None:
        buffer = self.model.replay_buffer
        if buffer.size() == 0:
            print("[Q-CALLBACK] Buffer vacío, no se puede calcular Q.")
            return

        obs = buffer.observations[:buffer.size()]
        actions = buffer.actions[:buffer.size()]
        rewards = buffer.rewards[:buffer.size()]

        q_values = []
        for o, a in zip(obs, actions):
            state_tensor = torch.tensor(o, dtype=torch.float32).unsqueeze(0)
            action_tensor = torch.tensor(a, dtype=torch.float32).unsqueeze(0)
            q1, q2 = self.model.critic(state_tensor, action_tensor)
            q_values.append(min(q1.item(), q2.item()))

        q_mean = np.mean(q_values)
        reward_sum = np.sum(rewards)
        env = self.training_env.envs[0]
        timestep_per_episode = env.get_wrapper_attr('timestep_per_episode')
        episode_num = self.num_timesteps // timestep_per_episode

        with open(self.csv_path, mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([episode_num, q_mean, reward_sum])

        print(f"[Q-CALLBACK] Episodio {episode_num} | Q promedio: {q_mean:.2f} | Recompensa acumulada: {reward_sum:.2f}")
