...
def DEFAULT_NUESTRO_DISCRETE_FUNCTION(action: int) -> np.ndarray:
...
def DEFAULT_NUESTROMULTIZONA_DISCRETE_FUNCTION(action: np.ndarray) -> np.ndarray:
    """
    Función de acción continua para Sinergym.
    Recibe directamente el vector de acción del agente (ej. SAC) y lo devuelve
    asegurando que sea np.float32.
 
    Parámetros
    ----------
    action : np.ndarray
        Vector continuo con los valores de setpoints (ej. calefacción y refrigeración).
 
    Retorna
    -------
    np.ndarray
        Vector de acción convertido a float32.
    """
    # SB3 puede devolver acciones como listas o arrays con otro dtype
