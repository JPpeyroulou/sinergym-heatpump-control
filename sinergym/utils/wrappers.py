...
class NormalizeAction(gym.ActionWrapper):
    ...
    def reverting_action(self,
        ...
        ret = self.real_space.low + (action - self.normalized_space.low) * self.scale
        ret = np.rint(ret).astype(np.float32)
        return ret
...
# -------------------------------------------- #
#           wrapper para prediccion de clima
# --------------------------------------------- #
...
class ForecastWrapper(gym.ObservationWrapper):
    """
    Añade un vector de predicción meteorológica de horizonte 'horizon' horas
    al vector de observación original.
    Espera un CSV con índice 'timestep' y columnas de clima (temp, humedad, etc.).
    """

    def __init__(self, env: gym.Env, forecast_csv: str, horizon: int = 24):
        super().__init__(env)
        self.horizon = horizon
        self.df = pd.read_csv(forecast_csv, index_col="timestep")
        orig_dim = env.observation_space.shape[0]
        feat_n = self.df.shape[1]
        new_dim = orig_dim + horizon * feat_n
    ...
    def observation(self, obs: Union[np.ndarray, Dict[str, float]]) -> np.ndarray:
        # Convierte dict -> vector si es necesario
        if isinstance(obs, dict):
            obs = np.fromiter(obs.values(), dtype=np.float32)

        t = getattr(self.env, "timestep", 0)
        block = self.df.loc[t : t + self.horizon - 1]

        # Relleno con última fila si no hay suficientes
        if len(block) < self.horizon:
            last = block.iloc[[-1]]
            pad  = pd.concat([last] * (self.horizon - len(block)), ignore_index=True)
            block = pd.concat([block.reset_index(drop=True), pad], ignore_index=True)

        forecast_vec = block.values.reshape(-1).astype(np.float32)
        return np.concatenate([obs, forecast_vec], axis=0)
